...
__global__ void matmul_kernel(const double *A, const double *BT, double *C,
  ...
  if (i < n && j < n) {
    double sum = 0.0;
    for (int p = 0; p < k; p++)
      sum += A[i * k + p] * BT[j * k + p]; // Use transposed B (BT)
...
__global__ void matmul_kernel_shared(const double *A, const double *BT,
  ...
  for (int t = 0; t < (k + TILE_WIDTH - 1) / TILE_WIDTH; ++t) {
    ...
    // Load transposed B (BT) into shared memory
    ...
    for (int p = 0; p < TILE_WIDTH; ++p)
      sum += tile_A[threadIdx.x][p] * tile_BT[threadIdx.y][p]; // Note access pattern change
...
// Transpose matrix B to improve memory access patterns
...
// Modified functions to use the transposed matrix
