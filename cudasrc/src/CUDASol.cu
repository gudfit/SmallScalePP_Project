...
__global__ void transpose_kernel(const double *B, double *BT, int k, int n) {
  ...
  row = blockIdx.x * BLOCK_DIM + threadIdx.y;
  col = blockIdx.y * BLOCK_DIM + threadIdx.x;
  
  if (row < n && col < k) 
    BT[row * k + col] = tile[threadIdx.x][threadIdx.y];
}
...
__global__ void matmul_kernel_shared(const double *A, const double *BT,
  ...
  for (int t = 0; t < (k + TILE_WIDTH - 1) / TILE_WIDTH; ++t) {
    ...
    if (col < n && t * TILE_WIDTH + tx < k)
      tile_BT[ty][tx] = BT[col * k + t * TILE_WIDTH + tx];
    else
      tile_BT[ty][tx] = 0.0;
